...
int wlan_set_chanlist_and_txpwrlimit(wifi_chanlist_t *chanlist, wifi_txpwrlimit_t *txpwrlimit)
{
    ...
    if (ret != WM_SUCCESS)
    {
        wlcm_e("[WiFi!] Cannot set Channel List");
        return ret;
    }
...
uint16_t wlan_get_beacon_period()
{
    ...
    if (ret != WM_SUCCESS)
    {
        wlcm_e("[WiFi!] cannot get network info");
        return 0;
    }
...
static int pscan_cb(unsigned int count)
{
    ...
    if (count == 0)
    {
        (void)PRINTF("[WiFi] networks not found\r\n");
        return 0;
    }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (err != 0)
        {
            (void)PRINTF("[WiFi!] Error: can't get scan res %d\r\n", i);
            continue;
        }
...
uint8_t wlan_get_dtim_period()
{
    ...
    {
        (void)PRINTF("[WiFi!] Error: scan request failed\r\n");
        os_semaphore_put(&wlan_dtim_sem);
...
void wlan_version_extended()
{
    ...
    (void)PRINTF("[WiFi] WLAN Driver Version   : %s\r\n", WLAN_DRV_VERSION);
    (void)PRINTF("[WiFi] WLAN Firmware Version : %s\r\n", version_str);
}
