...
static int wlan_event_callback(enum wlan_event_reason reason, void *data)
{
    ...
    switch (reason)
    {
        case WLAN_REASON_SUCCESS:
            WPL_GetIP(ip, 1);
            PRINTF("[WiFi] Connected to following BSS:");
            PRINTF("[WiFi] SSID = [%s], IP = [%s]\r\n", sta_network.ssid, ip);
        ...
        case WLAN_REASON_CONNECT_FAILED:
            PRINTF("[WiFi!] WLAN: connect failed\r\n");
        ...
        case WLAN_REASON_NETWORK_NOT_FOUND:
            PRINTF("[WiFi!] WLAN: network not found\r\n");
        ...
        case WLAN_REASON_NETWORK_AUTH_FAILED:
            PRINTF("[WiFi!] Network Auth failed\r\n");
        ...
        case WLAN_REASON_ADDRESS_FAILED:
            PRINTF("[WiFi!] failed to obtain an IP address\r\n");
        ...
        case WLAN_REASON_USER_DISCONNECT:
            PRINTF("[WiFi] Dis-connected from: %s\r\n", sta_network.ssid);
        ...
        case WLAN_REASON_INITIALIZATION_FAILED:
            PRINTF("[WiFi] app_cb: WLAN: initialization failed\r\n");
            break;
        ...
        case WLAN_REASON_UAP_SUCCESS:
            PRINTF("[WiFi] Soft AP started successfully\r\n");
        ...
        case WLAN_REASON_UAP_CLIENT_ASSOC:
            ...
            PRINTF("[WiFi] Associated with Soft AP\r\n");
            break;
        case WLAN_REASON_UAP_CLIENT_DISSOC:
            PRINTF("[WiFi] Client => ");
            print_mac((const char *)data);
            PRINTF("[WiFi] Dis-Associated from Soft AP\r\n");
            break;

        case WLAN_REASON_UAP_START_FAILED:
            PRINTF("[WiFi!]  Failed to start AP\r\n");
            break;
        case WLAN_REASON_UAP_STOP_FAILED:
            PRINTF("[WiFi!]  Failed to stop AP\r\n");
            break;
        case WLAN_REASON_UAP_STOPPED:
            wlan_remove_network(uap_network.name);
            PRINTF("[WiFi] Soft AP stopped successfully\r\n");
            if (xUapTaskNotify != NULL)
            {
                xTaskNotifyGive(xUapTaskNotify);
                xUapTaskNotify = NULL;
            ...
            break;
        default:
            PRINTF("[WiFi!] Unknown Wifi CB Reason %d\r\n", reason);
            break;
    }
...
int deepSleepOn(int state)
{
	if (state)
	{
		return wlan_deepsleepps_on();
	} else
	{
		return wlan_deepsleepps_off();
	}
}
...
int WPL_Start_AP(char *ssid, char *password, int chan)
{
    ...
    {
        PRINTF("[WiFi!] Password is too long!\r\n");
        __BKPT(0);
        return WPL_ERROR;
    }
...
static int WLP_process_results(unsigned int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            PRINTF("[WiFi!] The SSID_JSON is too small, can not fill all the SSIDS \r\n");
...
int WPL_StartDHCPServer(char *ip, char *net)
{
    ...
    PRINTF("[WiFi] This also starts DHCP Server with IP %s\r\n", current_ip);
    if (dhcp_server_start(net_get_uap_handle()))
    {
        PRINTF("[WiFi!] Error in starting dhcp server\r\n");
        return WPL_ERROR;
    }
    return WPL_SUCCESS;
}
